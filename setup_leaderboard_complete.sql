...
-- Run this in your Supabase SQL Editor to ensure leaderboard works
...
-- Step 3: Handle existing policies safely
DO $$
BEGIN
    -- Drop policy if it exists, then recreate it
    DROP POLICY IF EXISTS "Enable read access for leaderboard" ON public.user_quiz_attempts;
...
-- Step 4: Handle user_points policy
DO $$
BEGIN
    DROP POLICY IF EXISTS "Leaderboard access to user_points" ON public.user_points;
    EXCEPTION WHEN undefined_object THEN 
        -- Policy doesn't exist, that's fine
END;
$$;

CREATE POLICY "Leaderboard access to user_points" ON public.user_points
  FOR SELECT USING (true);
...
-- Step 6: Show table counts for verification
SELECT 'user_quiz_attempts count:' as table_name, COUNT(*) as count FROM public.user_quiz_attempts
UNION ALL
SELECT 'user_points count:' as table_name, COUNT(*) as count FROM public.user_points
UNION ALL
SELECT 'profiles count:' as table_name, COUNT(*) as count FROM public.profiles;

-- Step 7: Check if user_points table exists and has the right structure
